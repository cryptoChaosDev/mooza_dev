...
exports.router.post("/me/portfolio-file", upload.single('file'), async (req, res) => {
    try {
        const userId = authUserId(req);
        if (!userId)
            return res.status(401).json({ error: "Unauthorized" });
        ...
        const profile = await prisma.profile.update({
            ...
            data: {
                portfolioFile: buffer_1.Buffer.from(file.buffer),
...
exports.router.get("/me/portfolio-file", async (req, res) => {
    try {
        const userId = authUserId(req);
        if (!userId)
            return res.status(401).json({ error: "Unauthorized" });
        const profile = await prisma.profile.findUnique({ where: { userId } });
        const profAny = profile;
...
exports.router.post("/me/avatar", avatarUpload.single('avatar'), async (req, res) => {
    try {
        const userId = authUserId(req);
        if (!userId)
            return res.status(401).json({ error: "Unauthorized" });
...
exports.router.put("/me", async (req, res) => {
    const userId = authUserId(req);
    if (!userId)
        return res.status(401).json({ error: "Unauthorized" });
    // Validate and parse request body with Zod
    const profileSchema = zod_1.z.object({
        firstName: zod_1.z.string().min(1).max(50).transform(s => s.trim()),
        lastName: zod_1.z.string().min(1).max(50).transform(s => s.trim()),
        avatarUrl: zod_1.z.string().url().nullable().optional().or(zod_1.z.literal("")),
        bio: zod_1.z.string().optional().default("").transform(s => s.trim().substring(0, 1000)),
        workPlace: zod_1.z.string().optional().default("").transform(s => s.trim().substring(0, 100)),
        skills: zod_1.z.array(zod_1.z.string().min(1).max(50)).default([]).transform(arr => arr.map(s => s.trim()).filter(Boolean)),
        interests: zod_1.z.array(zod_1.z.string().min(1).max(50)).default([]).transform(arr => arr.map(s => s.trim()).filter(Boolean)),
        portfolio: zod_1.z.object({
            text: zod_1.z.string().optional().default("").transform(s => s.trim().substring(0, 500)),
            fileName: zod_1.z.string().optional(),
            fileUrl: zod_1.z.string().optional(),
        }).nullable().optional(),
        phone: zod_1.z.string().optional(),
        email: zod_1.z.string().email().optional(),
        city: zod_1.z.string().optional().default("").transform(s => s.trim().substring(0, 50)),
        country: zod_1.z.string().optional().default("").transform(s => s.trim().substring(0, 50)),
    });
...
// Add a friend
exports.router.post("/me/friends/:userId", async (req, res) => {
    const currentUserId = authUserId(req);
    if (!currentUserId)
        return res.status(401).json({ error: "Unauthorized" });
    ...
    try {
        ...
        const currentUserProfiles = await prisma.$queryRaw `
...
// Remove a friend
exports.router.delete("/me/friends/:userId", async (req, res) => {
    const currentUserId = authUserId(req);
    if (!currentUserId)
        return res.status(401).json({ error: "Unauthorized" });
    const targetUserIdStr = req.params.userId;
    ...
    `;
        ...
        // Remove friend
        const updatedFriends = currentFriends.filter((id) => id !== `user_${targetUserId}`);
        // Update profile using raw query
        await prisma.$executeRaw `
      UPDATE Profile 
      SET friendsCsv = ${updatedFriends.join(',')}
...
exports.router.get("/me/friends", async (req, res) => {
    const currentUserId = authUserId(req);
    if (!currentUserId)
        return res.status(401).json({ error: "Unauthorized" });
    try {
...
// Create a new post
exports.router.post("/me/posts", async (req, res) => {
    const userId = authUserId(req);
    if (!userId)
        return res.status(401).json({ error: "Unauthorized" });
    try {
...
exports.router.get("/me/posts", async (req, res) => {
    const userId = authUserId(req);
    if (!userId)
        return res.status(401).json({ error: "Unauthorized" });
    try {
...
exports.router.get("/posts", async (req, res) => {
    const userId = authUserId(req);
    if (!userId)
        return res.status(401).json({ error: "Unauthorized" });
    try {
...
// Update a post
exports.router.put("/me/posts/:postId", async (req, res) => {
    const userId = authUserId(req);
    if (!userId)
        return res.status(401).json({ error: "Unauthorized" });
    try {
...
// Delete a post
exports.router.delete("/me/posts/:postId", async (req, res) => {
    const userId = authUserId(req);
    if (!userId)
        return res.status(401).json({ error: "Unauthorized" });
    try {
...
// Toggle post like
exports.router.post("/posts/:postId/like", async (req, res) => {
    const userId = authUserId(req);
    if (!userId)
        return res.status(401).json({ error: "Unauthorized" });
    try {
