...
# Mooza VPS Setup Script
# This script downloads and prepares the automated deployment script
...
download_script() {
    log "Downloading automated deployment script..."
    
    # Try to download from GitHub
    if curl -fsSL -o /tmp/automated-deploy.sh "https://raw.githubusercontent.com/cryptoChaosDev/mooza_dev/master/deployment/automated-deploy.sh"; then
...
# Alternative method: Create script from this repository
create_from_repo() {
    log "Creating deployment script from repository..."
    
    cat > /usr/local/bin/mooza-deploy << 'EOF'
#!/bin/bash
...
EOF
    ...
    chmod +x /usr/local/bin/mooza-deploy
    success "Deployment script created and installed to /usr/local/bin/mooza-deploy"
}
...
show_instructions() {
    ...
    echo
    echo -e "${YELLOW}To deploy Mooza, run:${NC}"
    echo -e "  ${BLUE}sudo mooza-deploy${NC}"
    echo
    echo -e "${YELLOW}The deployment script will:${NC}"
    echo -e "  ✓ Update your system"
    echo -e "  ✓ Install all required dependencies"
    echo -e "  ✓ Install Node.js, Docker, and Docker Compose"
    echo -e "  ✓ Clone the Mooza repository"
    echo -e "  ✓ Build the frontend and backend"
    echo -e "  ✓ Configure Nginx and Docker Compose"
    echo -e "  ✓ Set up firewall rules"
    echo -e "  ✓ Deploy the application"
    echo -e "  ✓ Create systemd service for automatic startup"
    echo
    echo -e "${YELLOW}After deployment, your application will be available at:${NC}"
    echo -e "  ${GREEN}http://your-vps-ip${NC}"
}
...
main() {
    ...
    # Try to download first, fallback to creating from repo
    if download_script; then
        show_instructions
    else
        warning "Could not download script, creating from repository..."
        create_from_repo
        show_instructions
    fi
}
