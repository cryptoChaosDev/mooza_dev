...
export default function VkLoginButton({ onAuth, onError, disabled }: VkLoginButtonProps) {
  ...
  return (
    <button
      ...
      className="w-full flex items-center justify-center gap-2.5 px-4 py-3 bg-[#0077FF] hover:bg-[#0066EE] disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium rounded-xl transition-all shadow-lg shadow-[#0077FF]/20"
    >
      ...
      {loading ? 'Входим...' : 'Войти через ВКонтакте'}
    </button>
  );
}
