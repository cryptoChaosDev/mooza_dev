      ...
      <div className="max-w-7xl mx-auto px-4 pb-24 space-y-6">
          ...
          <div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {requests.map((request: any) => (
                <div
                  key={request.id}
                  className="group relative overflow-hidden bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-sm rounded-xl p-3.5 border border-slate-700/50 hover:border-primary-500/50 transition-all duration-300 shadow-md hover:shadow-primary-500/10"
                >
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <button onClick={() => navigate(`/profile/${request.requester.id}`)} className="flex-shrink-0">
                      {request.requester.avatar ? (
                        ...
                        <div className="w-10 h-10 bg-gradient-to-br from-primary-500 to-primary-700 rounded-xl flex items-center justify-center ring-2 ring-slate-700/50 group-hover:ring-primary-500/50 transition-all">
        ...
        {friendsLoading ? (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-sm rounded-xl p-3.5 border border-slate-700/50 animate-pulse">
          ...
          <div>
            <h3 className="text-sm font-bold text-white mb-3">
              Все друзья <span className="text-slate-400 font-normal">({friends.length})</span>
            </h3>
                ...
                >
                  ...
                  <div className="flex flex-col items-center text-center">
                    ...
                    <button onClick={() => navigate(`/profile/${friend.id}`)} className="mb-3">
                      {friend.avatar ? (
                        <img
                          ...
                          className="w-14 h-14 rounded-xl object-cover ring-2 ring-slate-700/50 group-hover:ring-primary-500/50 transition-all"
                        />
                      ) : (
                        <div className="w-14 h-14 bg-gradient-to-br from-primary-500 to-purple-600 rounded-xl flex items-center justify-center ring-2 ring-slate-700/50 group-hover:ring-primary-500/50 transition-all">
                    ...
                    <button
                      ...
                      className="font-semibold text-white text-sm hover:text-primary-400 transition-colors truncate w-full"
                    >
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-xl rounded-2xl border border-slate-700/50 shadow-xl">
            <div className="relative text-center py-10 px-6">
              ...
              <p className="text-slate-400 text-sm">Найдите музыкантов в поиске</p>
              <button
                onClick={() => navigate('/search')}
                className="mt-4 px-4 py-2 bg-gradient-to-r from-primary-500 to-primary-600 hover:from-primary-600 hover:to-primary-700 text-white rounded-lg text-sm font-medium transition-all shadow-lg shadow-primary-500/25 hover:shadow-primary-500/40"
              >
                Найти друзей
              </button>
            </div>
