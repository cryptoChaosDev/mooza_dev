...
import {
  ...
  UserPlus, Trash2, Search, Users, Music, Link2,
} from 'lucide-react';
...
export default function GroupPage() {
  ...
  const { data: group, isLoading, isError } = useQuery({
    queryKey: ['artist', id],
    queryFn: async () => { const { data } = await artistAPI.getArtist(id!); return data; },
    enabled: !!id,
  });
  ...
  const { data: friendsList = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data as { friendshipId: string; user: { id: string; firstName: string; lastName: string; avatar?: string } }[]).map(f => f.user);
    },
    ...
    <div className="min-h-screen bg-slate-950 pb-28">
      ...
      {isEditing && (
        <>
            ...
            >
              ...
              <div className="flex-1 min-h-0 overflow-y-auto px-4 py-4 space-y-4">
                <div>
                  ...
                  <input className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-primary-500" value={form.name} onChange={e => set('name', e.target.value)} placeholder="Название группы" />
                </div>
                <div>
                  <label className="block text-xs text-slate-500 mb-1">Тип</label>
                  <button type="button" onClick={() => setTypeSheetOpen(true)} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-sm text-left flex justify-between items-center">
                    <span className={form.type ? 'text-white' : 'text-slate-500'}>{form.type ? TYPE_LABELS[form.type] : 'Выбрать тип'}</span>
                    <span className="text-slate-500 text-xs">▾</span>
                  </button>
                </div>
                <div>
                  <label className="block text-xs text-slate-500 mb-1">Город</label>
                  <input className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-primary-500" value={form.city} onChange={e => set('city', e.target.value)} placeholder="Москва" />
                </div>
                <div>
                  <label className="block text-xs text-slate-500 mb-1">Описание</label>
                  <textarea rows={4} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-primary-500 resize-none" value={form.description} onChange={e => set('description', e.target.value)} placeholder="О группе..." />
                </div>
                <div>
                  <label className="block text-xs text-slate-500 mb-1">Жанры</label>
                  <button type="button" onClick={() => setGenreSheetOpen(true)} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-sm text-left flex justify-between items-center">
                    <span className={form.genreIds.length ? 'text-white' : 'text-slate-500'}>
                      {form.genreIds.length ? genreOptions.filter(g => form.genreIds.includes(g.id)).map(g => g.name).join(', ') : 'Выбрать жанры'}
                    </span>
                    <span className="text-slate-500 text-xs">▾</span>
                  </button>
                </div>
                <div>
                  ...
                  <input className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-primary-500" value={form.bandLink} onChange={e => set('bandLink', e.target.value)} placeholder="https://band.link/..." />
                </div>
                <div>
          ...
          <SelectSheet isOpen={typeSheetOpen} onClose={() => setTypeSheetOpen(false)} title="Тип группы" options={TYPE_OPTIONS} selectedIds={form.type} onSelect={v => { set('type', v as string); setTypeSheetOpen(false); }} mode="single" searchable={false} height="auto" />
          <SelectSheet isOpen={genreSheetOpen} onClose={() => setGenreSheetOpen(false)} title="Жанры" options={genreOptions} selectedIds={form.genreIds} onSelect={v => { set('genreIds', v as string[]); }} mode="multiple" showConfirm height="full" />
        </>
      )}
      ...
      {showInviteModal && (
        <div className="fixed inset-0 z-50 bg-slate-950 flex flex-col">
          <div className="flex items-center gap-3 px-4 py-4 border-b border-slate-800 flex-shrink-0 bg-slate-900/80 backdrop-blur">
            <button onClick={() => setShowInviteModal(false)} className="p-2 text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition-colors"><ArrowLeft size={20} /></button>
            <h2 className="text-base font-semibold text-white flex-1">Пригласить участника</h2>
            <button onClick={() => inviteMut.mutate()} disabled={!inviteFriendId || !inviteProfessionId || inviteMut.isPending} className="px-4 py-2 bg-primary-600 hover:bg-primary-500 disabled:bg-slate-700 disabled:text-slate-500 text-white rounded-xl text-sm font-semibold transition-all flex items-center gap-2">
              {inviteMut.isPending && <Loader2 size={14} className="animate-spin" />}Пригласить
            </button>
          </div>
          <div className="flex-1 overflow-y-auto">
            <div className="px-4 pt-4 pb-2">
              ...
              <div className="relative mb-2">
                ...
                <input value={inviteFriendSearch} onChange={e => setInviteFriendSearch(e.target.value)} placeholder="Поиск по имени..." className="w-full pl-9 pr-3 py-2 bg-slate-800 border border-slate-700 rounded-xl text-sm text-white placeholder-slate-500 outline-none focus:border-primary-500" />
              </div>
            ...
            <div className="px-4 pb-4">
              ...
              <div className="relative mb-2">
                <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
                <input value={inviteProfSearch} onChange={e => setInviteProfSearch(e.target.value)} placeholder="Гитарист, вокалист, барабанщик..." className="w-full pl-9 pr-3 py-2 bg-slate-800 border border-slate-700 rounded-xl text-sm text-white placeholder-slate-500 outline-none focus:border-primary-500" />
              </div>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="relative group">
          ...
          {isOwner && (
            <>
              <button onClick={() => bannerInputRef.current?.click()} className="absolute bottom-3 right-3 flex items-center gap-1.5 px-2.5 py-1.5 bg-black/50 hover:bg-black/70 backdrop-blur-sm text-white/80 hover:text-white rounded-lg text-xs font-medium transition-all opacity-0 group-hover:opacity-100">
                <Camera size={12} />Сменить фон
              </button>
              <input ref={bannerInputRef} type="file" accept="image/*" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) uploadBannerMut.mutate(f); e.target.value = ''; }} />
            </>
          )}
        </div>
        ...
        <div className="px-4">
          ...
          <div className="flex items-end justify-between -mt-14 mb-4">
            <div className="relative z-10 flex-shrink-0">
              <div className="w-28 h-28 rounded-2xl ring-4 ring-slate-950 overflow-hidden bg-gradient-to-br from-primary-600 to-purple-700 flex items-center justify-center shadow-2xl">
                {avatarSrc
              ...
              {isOwner && (
                <>
                  <button onClick={() => avatarInputRef.current?.click()} className="absolute bottom-0.5 right-0.5 bg-primary-500 hover:bg-primary-600 text-white p-2 rounded-full shadow-lg transition-all border-2 border-slate-950">
                    <Camera size={13} />
                  </button>
                  <input ref={avatarInputRef} type="file" accept="image/*" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) uploadAvatarMut.mutate(f); e.target.value = ''; }} />
                </>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 mb-1">
              <ShareButton url={`/groups/${id}`} title={group?.name} className="p-2 bg-slate-800/80 hover:bg-slate-700 border border-slate-700/60 text-slate-400 hover:text-white rounded-xl transition-all" iconSize={16} />
              {isOwner && (
                <>
                  <button onClick={() => setIsEditing(true)} className="p-2 bg-slate-800/80 hover:bg-slate-700 border border-slate-700/60 text-slate-400 hover:text-white rounded-xl transition-all" title="Редактировать">
                    <Edit3 size={16} />
                  </button>
                  <button onClick={() => setShowDeleteConfirm(true)} className="p-2 bg-slate-800/80 hover:bg-red-500/15 border border-slate-700/60 hover:border-red-500/30 text-slate-400 hover:text-red-400 rounded-xl transition-all" title="Удалить группу">
                    <Trash2 size={16} />
                  </button>
                </>
              )}
              {currentUser && !isOwner && (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium transition-all border ${
                    group.isFollowed
                      ? 'bg-slate-800/80 border-slate-700/60 text-slate-300 hover:text-red-400 hover:border-red-500/30'
                      : 'bg-primary-600/20 border-primary-500/40 text-primary-300 hover:bg-primary-600/30'
                  }`}
                >
          ...
          <div className="flex items-center gap-2 flex-wrap mb-0.5">
            ...
            {group.status === 'VERIFIED' && (
              <span className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium bg-green-500/10 text-green-400 border border-green-500/25">
                <ShieldCheck size={11} />Верифицирована
              </span>
            )}
            {group.status === 'PENDING' && (
              <span className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium bg-amber-500/10 text-amber-400 border border-amber-500/25">
                <Clock size={11} />На модерации
              </span>
            )}
            {group.status === 'APPROVED' && (
              <span className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium bg-sky-500/10 text-sky-400 border border-sky-500/25">
          ...
          <div className="flex items-center gap-2 mb-5 flex-wrap">
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800/60 rounded-xl">
              <span className="text-sm font-bold text-white">{group.followersCount ?? 0}</span>
              <span className="text-xs text-slate-500">{plural(group.followersCount ?? 0, 'подписчик', 'подписчика', 'подписчиков')}</span>
            </div>
            {acceptedMembers.length > 0 && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800/60 rounded-xl">
                <span className="text-sm font-bold text-white">{acceptedMembers.length}</span>
                <span className="text-xs text-slate-500">{plural(acceptedMembers.length, 'участник', 'участника', 'участников')}</span>
              </div>
            )}
          </div>
          ...
          <div className="space-y-3">
            ...
            {isOwner && (group.status === 'DRAFT' || group.status === 'REJECTED') && (
              <div className={`rounded-2xl border p-4 ${group.status === 'REJECTED' ? 'bg-red-500/5 border-red-500/20' : 'bg-slate-900/60 border-slate-800/60'}`}>
                ...
                <p className="text-xs text-slate-400 mb-3">
                  {group.status === 'DRAFT' ? 'Группа в черновике. Отправьте на модерацию, чтобы появиться в каталоге.' : 'Группа отклонена. Исправьте данные и отправьте снова.'}
                </p>
                <button onClick={() => submitMut.mutate()} disabled={submitMut.isPending} className="flex items-center gap-1.5 px-3 py-2 bg-primary-600 hover:bg-primary-500 disabled:opacity-50 text-white text-xs font-medium rounded-xl transition-colors">
            ...
            {(group.genres?.length ?? 0) > 0 && (
              <div className="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  <Music size={14} className="text-primary-400" />
                  <span className="text-sm font-semibold text-white">Жанры</span>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {group.genres.map((g: { id: string; name: string }) => (
                    <span key={g.id} className="px-2.5 py-1 bg-primary-500/10 border border-primary-500/25 text-primary-300 rounded-lg text-xs font-medium">
                      {g.name}
                    </span>
                  ))}
                </div>
            ...
            <div className="bg-slate-900/60 border border-slate-800/60 rounded-2xl overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-800/60">
                ...
                {(group.members?.length ?? 0) > 0 && <span className="text-xs text-slate-500">{group.members.length}</span>}
                {isOwner && (
                  <button onClick={() => setShowInviteModal(true)} className="ml-auto flex items-center gap-1 text-xs text-primary-400 hover:text-primary-300 transition-colors">
              ...
              <div className="p-3">
                {(group.members?.length ?? 0) === 0 ? (
                  <p className="text-xs text-slate-600 italic py-1">Участников пока нет. Пригласите друзей.</p>
                ) : (
                  <div className="space-y-2">
                    {group.members.map((m: any) => (
                      <div key={m.membershipId} className="flex items-center gap-3 p-2.5 rounded-xl bg-slate-800/40 border border-slate-700/40 hover:border-slate-600/60 transition-colors">
                        ...
                        <div className="flex-1 min-w-0 cursor-pointer" onClick={() => navigate(`/profile/${m.id}`)}>
                          ...
                          <p className="text-xs text-slate-500 truncate">{m.isOwner ? 'Основатель' : (m.profession?.name ?? '—')}</p>
                        </div>
            ...
            {currentUser && isOwner && (
              <button
                ...
                className={`w-full py-3 rounded-xl text-sm font-semibold transition-colors border ${
                  group.isFollowed
                    ? 'bg-slate-800/60 text-slate-300 border-slate-700/60 hover:text-red-400 hover:border-red-500/30'
                    : 'bg-primary-600/20 text-primary-300 border-primary-500/30 hover:bg-primary-600/30'
                }`}
              >
                {group.isFollowed ? 'Отписаться от группы' : 'Подписаться на группу'}
              </button>
            )}
